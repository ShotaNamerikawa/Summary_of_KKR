...
(未完成)
## Purpose
KKR(-Green関数)法について勉強した結果、特にグリーン関数の計算手法についてまとめることを目的とする。
...
KKR-Green関数法ではポテンシャルをオーバーラップしないように
```math
V = \sum_{n}{v_n (\vec{r})}
```
```math
...
## グリーン関数の記法に関するメモ
OaklidgeとJulichの人たちは異なるグリーン関数の表記方法を用いる。上で示したのはOaklidge notationである。それらの変換に関してはEbert 2011に記載がある。
